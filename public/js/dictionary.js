...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_sweetalert__ = __webpack_require__(13);
...
window.showTranslation = function (word, translation) {
...
// Spinner
window.showSpinner = function (button) {

    button.prop('disabled', true);

    button.append($('<div />').addClass('spinner-container').append($('<i />').addClass('material-icons spinner').html('autorenew')));
};
...
window.showModal = function (title, content) {
    ...
    var modalPopup = $('<div />').addClass('modal').attr({
        'id': 'deleteCollection',
        'tabindex': '-1',
        'role': 'dialog'
    }).append($('<div />').addClass('modal-dialog modal-dialog-centered').attr('role', 'document').append($('<div />').addClass('modal-content').append($('<div / >').addClass('modal-header').append($('<h5 />').addClass('modal-title text-primary').html(title)).append($('<button />').addClass('close').attr({
        'data-dismiss': 'modal',
        'area-label': 'Close'
    }).append($('<span />').attr('area-hidden', 'true').html('&times;')))).append($('<div />').addClass('modal-body').append(content)).append($('<div />').addClass('modal-footer').append(footer))));
...
$(document).ready(function () {
    ...
    $('#search_form').on('submit', function (event) {
        ...
        $('#descriptions_list tr').each(function () {
            ...
            var word = $('td:eq(0)', $(this)).text();
            var translation = $('td:eq(1)', $(this)).text();

            var notFound = word.toLowerCase().indexOf(search) === -1 && translation.toLowerCase().indexOf(search) === -1;
    ...
    $('#describe_word_frm').on('submit', function (event) {
        ...
        axios.post('/dictionary/extend', qs.stringify(credentials)).then(function () {

            hideSpinner(button);

            clearForm($(_this));

            showMessage('Word successfully added to the dictionary.').then(function () {
                return $('input[type="text"]:eq(0)').focus();
            });
        }).catch(function (error) {

            hideSpinner(button);

            processErrors(error, $(_this));
        });
    });
